...
fn device_factory_reset() {
    with_ram_clients_and_preserve(
        ...
        |[mut client1, mut client2]| {
            ...
            assert!(
                syscall!(client1.entry_metadata(Location::Internal, "to_save_internal".into()))
                    .metadata
                    .is_some()
            );
            assert!(
                syscall!(client1.entry_metadata(Location::External, "to_save_external".into()))
                    .metadata
                    .is_some()
            );
            assert!(
                syscall!(client1.entry_metadata(Location::Volatile, "to_save_volatile".into()))
                    .metadata
                    .is_some()
            );
            assert!(
