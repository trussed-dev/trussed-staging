...
fn test_write_all(location: Location) {
    with_ram_client("test chunked", |mut client| {
        let path = path!("foo");
        utils::write_all(&mut client, location, path.into(), &[48; 1234], None, None).unwrap();

        let data = syscall!(client.start_chunked_read(location, path.into())).data;
...
fn test_write_all_small(location: Location) {
    with_ram_client("test chunked", |mut client| {
        let path = path!("foo2");
        utils::write_all(&mut client, location, path.into(), &[48; 1023], None, None).unwrap();
...
fn filesystem() {
    with_ram_client("chunked-tests", |mut client| {
        ...
        assert!(
            syscall!(client.entry_metadata(Location::Internal, path.into()))
                .metadata
                .is_none(),
        );

        let data = Bytes::from_slice(b"test data").unwrap();
        syscall!(client.write_file(Location::Internal, path.into(), data.clone(), None,));

        let recv_data = syscall!(client.read_file(Location::Internal, path.into())).data;
        assert_eq!(data, recv_data);

        // ======== CHUNKED READS ========
        let first_data = syscall!(client.start_chunked_read(Location::Internal, path.into()));
        ...
        // ======== CHUNKED WRITES ========
        syscall!(client.start_chunked_write(Location::Internal, path.into(), None));
        ...
        let metadata = syscall!(client.entry_metadata(Location::Internal, path.into()))
            .metadata
            .unwrap();
        ...
        let partial_data = syscall!(client.start_chunked_read(Location::Internal, path.into()));
        ...
        assert!(
            try_syscall!(client.remove_file(Location::Internal, path!("bad_name").into())).is_err()
        );
        let metadata = syscall!(client.entry_metadata(Location::Internal, path.into()))
            .metadata
            .unwrap();
        ...
        syscall!(client.remove_file(Location::Internal, path.into()));
        assert!(
            syscall!(client.entry_metadata(Location::Internal, path.into()))
                .metadata
                .is_none(),
